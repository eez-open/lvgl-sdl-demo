...
static void keypad_read(lv_indev_drv_t *indev_drv_gamepad, lv_indev_data_t *data)
{
    ...
    static uint32_t last_spec_key = 0;
    static int repeat_last_spec_key = 0;
    ...
    if (SDL_PollEvent(&e))
    {
            ...
            if (e.key.repeat) {
                ...
                for (int i = 0; i < (sizeof(lvgl_keyboard_map) / sizeof(keyboard_map_t)); i++)
                {
                    if (lvgl_keyboard_map[i].sdl_map == e.key.keysym.sym)
                    {
                        ...
                        last_spec_key = (e.type == SDL_KEYDOWN) ? data->key : 0;
        ...
        if (e.type == SDL_TEXTINPUT) {
            char *p = e.text.text;
            while (*p && keyboard_buffer_index < sizeof(keyboard_buffer)) {
                keyboard_buffer[keyboard_buffer_index++] = *p++;
            }
        }
    }

    if (data->key == 0) {
        if (repeat_last_spec_key == 1) {
            data->key = last_spec_key;
            data->state = LV_INDEV_STATE_RELEASED;
            repeat_last_spec_key = 2;
        } else if (repeat_last_spec_key == 2) {
            data->key = last_spec_key;
            data->state = LV_INDEV_STATE_PRESSED;
            repeat_last_spec_key = 0;
        } else if (last_key != 0) {
            data->key = last_key;
            data->state = LV_INDEV_STATE_RELEASED;
            last_key = 0;
        } else if (keyboard_buffer_index > 0) {
            data->key = keyboard_buffer[0];
            data->state = LV_INDEV_STATE_PRESSED;
            keyboard_buffer_index--;
            memmove(keyboard_buffer, keyboard_buffer + 1, keyboard_buffer_index);
            last_key = 0;
        }
    }

    //Is there more input events?
    data->continue_reading = (SDL_PollEvent(NULL) != 0) || last_key || keyboard_buffer_index > 0 || repeat_last_spec_key;
}
